...
@endphp
<dialog id="{{ $id }}" {{ $attributes->merge(['class' => 'rounded-lg p-0 backdrop:bg-black/40 w-full max-w-sm border border-neutral-200 dark:border-neutral-800']) }}>
    <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-xl p-5 flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
            @if($title)<h2 class="text-base font-semibold mb-1">{{ $title }}</h2>@endif
